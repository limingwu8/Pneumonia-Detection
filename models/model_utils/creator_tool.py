...
from models.model_utils.bbox_tools import bbox2loc, bbox_iou, loc2bbox
...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label,
        ...
        :func:`model.model_utils.bbox_tools.bbox2loc`.
...
class AnchorTargetCreator(object):
    ...
    def __call__(self, bbox, anchor, img_size):
        ...
        * :math:`R` is the number of bounding boxes.
        ...
        n_anchor = len(anchor)
        # Ignore the bounding boxes outside the image.
    ...
    def _create_label(self, inside_index, anchor, bbox):
        """
        Random choose 250 anchors from which have IOU smaller than 0.3 as negative samples
        From which IOU larger than 0.7 as positive samples.
        :param inside_index:
        :param anchor:
        :param bbox:
        :return:
...
class MaskTargetCreator():
    ...
    def __call__(self, masks):
        ...
        for i in range(len(masks)):
            ...
            # print(mask.shape)
            mask = transform.resize(mask, (14, 14), preserve_range=False, mode='constant')
...
class ProposalCreator:
    ...
    def __call__(self, loc, score,
        ...
        # Clip predicted boxes to image. Remove the rois if outbounds of the image(if the position is negative).
...
# def plot_anchor_bbox(anchor, bbox):
#     import matplotlib.patches as patches
#     import matplotlib.pyplot as plt
#     ax = plt.subplot(111)
#
#     ax.fill(-1000,-1000,1000,1000,'white')
#
#     for i in range(anchor.shape[0]):
#         rect = patches.Rectangle((anchor[i][1], anchor[i][0]), anchor[i][3] - anchor[i][1], anchor[i][2] - anchor[i][0],
#                                  linewidth=1, edgecolor='r', facecolor='none')
#         ax.add_patch(rect)
#     for i in range(bbox.shape[0]):
#         rect = patches.Rectangle((bbox[i][1], bbox[i][0]), bbox[i][3] - bbox[i][1], bbox[i][2] - bbox[i][0],
